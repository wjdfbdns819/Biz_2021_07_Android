...
public interface WordDao {
     ...
     *  
     *  List<DTO> 타입의 method를 사용하면 100개 데이터를 모두 읽이서
     *   list에 담아서 메모리에 저장을 한다
     *   이는 메모리에 부담이 되고, 디바이스 시스템에 무리를 줄수도 있다
     *   
     *  하지만 LiveData로 감싼 리스트는 안드로이드 OS 차원에서
     *   적절히 메모리를 분배하여 리스트를 관리해주는 효과가 있다
     * 
     *  리스트를 보고있는 동안에 새로운 데이터가 insert 되었다면
     *   일반적인 환경에서는 다시 selectAll()을 수행하여 데이터를 가져온다
     *   
     *  LiveData로 감싼 리스트는 새로 추가된 데이터만 select하여
     *   기존의 list에 추가해 준다
     */
    @Query("SELECT * FROM tbl_word ORDER BY word")
    public LiveData<List<WordDTO>> selectAll(); // 전체 불러오기

    @Insert(onConflict = OnConflictStrategy.IGNORE) // insertOrUpdate와 비슷한 일을 수행함
    public void insert(WordDTO wordDTO); // 추가하기
