...
/**
 * RoomDatabase를 상속받아 DB Connection, Session을 만드는 클래스임
 *  이 클래스는 반드시 abstract 키워드를 추가하여
 *  추상 클래스로 선언을 해야한다
 *  
 * 일부는 직접 코드를 구현하는 method를 포함하고,
 *  일부는 interface 처럼 코드가 구현되지 않은 method를 함께 포함하는 클래스이다
 */
 ...
 * 
 * version 항목의 내용
 *  혹시 사용 과정에서 table에 변경사항이 발생할 수 있다
 *   여기서는 table에 변경이 이루어지면 WordDTO 클래스에 칼럼들을
 *   변경하는 작업을 수행한다
 *   그리고, version 넘버를 현재 값보다 큰 값으로 변경한다
